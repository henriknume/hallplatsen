package ex.nme.hallplatsen;

/**
 * Created by nm2 on 2017-06-12
 */

public class Constants {
    public static String BASE_URL = "https://api.vasttrafik.se/";

    public static final String KEY = "THxu3Y2FWVolNrB3rZAz7MX3H7Ya" ;
    public static final String SECRET = "ttbX3jNBaS2Ls72Ts3TNqIl6xo4a" ;
    public static final String DEVICE_ID = "device_555" ;

    public static final String AUTH = "Basic VEh4dTNZMkZXVm9sTnJCM3JaQXo3TVgzSDdZYTp0dGJYM2pOQmFTMkxzNzJUczNUTnFJbDZ4bzRh";
}
